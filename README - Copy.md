...
The Node.js web API itself is protected using the [passport-azure-ad](https://github.com/AzureAD/passport-azure-ad) plug-in for [Passport.js](http://www.passportjs.org/).

The Node.js API, on its hand, calls MS Graph to acquire the user info. The authentication is handled by the [Microsoft Authentication Library for Node.js](https://github.com/AzureAD/microsoft-authentication-library-for-js/blob/dev/lib/msal-node/README.md).
...
1. If the access token is valid, there is call done to **MS Graph** to get the user information.
1. The MS Graph access token is acquired using a client credentials flow with the **MSAL Node**.
1. The protected web API responds with the user information returned from **MS Graph**.
